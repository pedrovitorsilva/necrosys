...
def imprimir(mensagem: str, cor: str):
    ...
    print(colored.fg('black'), colored.bg(cor), mensagem, colored.attr('reset'))
...
def esta_na_foto(caracteristicas, fotos_ja_registradas: list[str]):
    ...
    for foto in fotos_ja_registradas:
        ...
        reconhecimentos = fr.compare_faces(caracteristicas, caracteristicas_foto_atual)
...
def reconhecer_falecidos(foto_escaneada: str, falecidos_registrados: dict) -> Tuple[bool, list]:   # type: ignore
    '''
...
def verificar_rostos(simulacao: simpy.Environment, falecidos_registrados: dict, pessoas_reconhecidas: list[str], causas_da_morte: dict):
    ...
    while True:
        imprimir(f"Atendimento feito. Rosto sendo visto em {simulacao.now}...", "light_blue")

        rostos = scanner_foto()
        ocorreu_reconhecimento, novos_reconhecidos = reconhecer_falecidos(rostos, falecidos_registrados)
        
        if ocorreu_reconhecimento:
            print("Pessoa(s) reconhecida(s):")
            print([pessoa['nome'] for pessoa in novos_reconhecidos])
            print("\n")
            registrar_reconhecido(novos_reconhecidos, pessoas_reconhecidas,causas_da_morte)
...
def relatorio_causas_morte(simulacao: simpy.Environment, causas_morte: dict):
    ...
    '''
    while True:
        relatorio_ordenado = dict(sorted(causas_morte.items(), key=lambda item: item[1], reverse=True))
...
def verificar_disponibilidade_legista(simulacao: simpy.Environment, legista_ocupado:bool):
    ...
    while True:
        if random.randint(1,100) <= PROBABILIDADE_DISPONIBILIDADE_LEGISTA and legista_ocupado:
            ...
            imprimir(f"Verificando status do legista em {simulacao.now}:","spring_green_4")
            print("Legista terminou o procedimento e está disponível!!")
        else:
            legista_ocupado = True
            imprimir(f"Verificando status do legista em {simulacao.now}:","red_3a" )
...
def receber_orgaos(simulacao: simpy.Environment, orgaos: list[int]):
    ...
    while True:
        if random.randint(1, 100) <= PROBABILIDADE_RECEBIMENTO_ORGAOS:
            ...
            imprimir(f"{novos_orgaos} órgão(s) recebido(s) com sucesso em {simulacao.now}!", "blue_violet")
...
def doar_orgaos(simulacao: simpy.Environment, orgaos: list[int]):
    ...
    while True:
        if orgaos[0] <= 0:
            imprimir(f"Não há mais órgaos para doar em {simulacao.now}...", "red_3a")
        else:
            if random.randint(1, 100) <= PROBABILIDADE_DOACAO_ORGAOS:
                novos_orgaos = random.randint(1, orgaos[0])
                orgaos[0] -= novos_orgaos
                imprimir(f"{novos_orgaos} órgão(s) doado(s) com sucesso em {simulacao.now}!", "medium_spring_green")
...
if __name__ == "__main__":
    ...
    if houve_inicializacao:
        ...
        simulacao.process(verificar_rostos(simulacao, falecidos_registrados, pessoas_reconhecidas, causas_da_morte))

        simulacao.process(relatorio_causas_morte(simulacao, causas_da_morte))

        simulacao.process(verificar_disponibilidade_legista(simulacao,legista_ocupado))

        simulacao.process(receber_orgaos(simulacao,orgaos_disponiveis))
        simulacao.process(doar_orgaos(simulacao,orgaos_disponiveis))

        simulacao.run(until=4000)